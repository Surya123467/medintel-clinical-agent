...
def run_agent(mrn,message):
    ...
    if not state.get("patient"): return {"answer":"Patient not found.","clinician_summary":None,"patient_explanation":None,"tools_used":state.get("tools_used",[])}
    return {"answer":state["clinician_summary"],"clinician_summary":state["clinician_summary"],"patient_explanation":state["patient_explanation"],"tools_used":state["tools_used"]}
